...
describe("PaymentSuccessScreen copy per phase", () => {
  ...
  it("does NOT claim to be crediting a wallet nobody was asked about", () => {
    ...
    // ...and nothing may imply something went WRONG either. This phase is
    // every card top-up in production while the signed checkout is off, so
    // falling through to the ⚠ glyph put an 80px warning triangle over
    // "Payment received" for customers whose payment was fine — a support
    // ticket generated by an icon. The COLOUR already got this right
    // (colors.warning, not colors.error); the glyph did not.
    expect(queryByText("⚠")).toBeNull()
    expect(queryByText("ℹ")).not.toBeNull()
