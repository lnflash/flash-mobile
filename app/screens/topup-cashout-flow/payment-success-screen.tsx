          ...
          {checking ? (
            ...
            >
              {/* A clock says "on its way". Only `pending` is — held, failed,
                  ...
                  `unaskable` needs its own GLYPH for the same reason it has its
                  own colour. It is the phase EVERY card top-up lands in while
                  the signed checkout is off, so a warning triangle 80px tall
                  over the headline "Payment received" is what a successful
                  payer sees in production today — a support ticket generated by
                  an icon, for a payment that went fine. `ℹ` says what this
                  phase actually is: nothing was asked, so there is nothing to
                  warn about. */}
              {credited ? "✓" : claimsCredit ? "⏱" : unaskable ? "ℹ" : "⚠"}
            </Text>
          )}
