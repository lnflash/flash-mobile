...
export const CashoutDetailsScreen: React.FC<Props> = ({ navigation }) => {
  const {
    theme: { colors },
  } = useTheme()
  ...
  const [fromWalletCurrency, setFromWalletCurrency] = useState<WalletCurrency>("USD")
  ...
  const cashoutWalletId: Pick<Wallet, "id" | "walletCurrency" | "balance"> = {
    id: "25d80bca-065c-4179-b96f-dc1ce99024e7",
    walletCurrency: "USD",
    balance: 0,
  }
  ...
  console.log("cashoutWalletId: ", cashoutWalletId)
  const moveToNextScreen = () => {
    if (usdWallet && btcWallet) {
      navigation.navigate("cashoutConfirmation", {
        toWallet: cashoutWalletId,
        ...
        <View style={styles.fieldContainer}>
          <View style={styles.percentageLabelContainer}>
            <Text style={styles.percentageFieldLabel}>
              {LL.CashoutDetailsScreen.amountToCashout()}
            </Text>
          </View>
