...
const BackupSeedPhrase = ({ navigation }) => {
    ...
    const renderItemHandler = ({ item, index }) => {
        return (<SeedPhrase marginRight={index % 2 === 0} style={{ backgroundColor: mode === "dark" ? "#5b5b5b" : "#ededed" }}>
    ...
    return (<Wrapper style={{ backgroundColor: colors.white }}>
      <Container>
        ...
        <react_native_1.FlatList data={seedPhrase} numColumns={2} renderItem={renderItemHandler} columnWrapperStyle={{ justifyContent: "space-between" }} scrollEnabled={false} style={{ marginTop: 20 }}/>
      </Container>
      <ButtonsWrapper>
        ...
        <buttons_1.PrimaryBtn label={LL.BackupSeedPhrase.verify()} onPress={onVerify} btnStyle={{ marginBottom: bottom || 10 }}/>
      </ButtonsWrapper>
    </Wrapper>);
};
exports.default = BackupSeedPhrase;
...
const ButtonsWrapper = native_1.default.View `
  padding-top: 10px;
`;
//# sourceMappingURL=BackupSeedPhrase.js.map