    ...
    try {
      ...
      if (result === RESULTS.GRANTED) {
        ...
        Alert.alert(
          ...
          LL.common.contactsPermissionDeniedMessage?.() || "Contacts permission has been denied. Please enable it in Settings.",
          [
    ...
    try {
      Contacts.getAll()
        ...
        .catch((error) => {
          ...
          Alert.alert(
            LL.common.error?.() || "Error",
            LL.common.errorLoadingContacts?.() || "Failed to load contacts. Please try again."
          )
        })
    } finally {
      setLoading(false)
  ...
  const renderContact = ({ item }: { item: Contact }) => {
    ...
    if (totalContacts === 1) {
      ...
      if (item.phoneNumbers.length > 0) {
        parts.push(`${item.phoneNumbers.length} phone${item.phoneNumbers.length > 1 ? 's' : ''}`)
      }
      if (item.emailAddresses.length > 0) {
        parts.push(`${item.emailAddresses.length} email${item.emailAddresses.length > 1 ? 's' : ''}`)
      }
    ...
    return (
      ...
      >
        <ListItem.Content>
          ...
          <ListItem.Subtitle style={styles.contactPhone}>{contactDisplay}</ListItem.Subtitle>
        </ListItem.Content>
        <ListItem.Chevron />
      </ListItem>
    )
              ...
              <View style={styles.emptyContainer}>
                <Text style={styles.emptyText}>
                  {searchText
                    ...
                    : LL.common.noContactsWithPhone?.() || "No contacts with phone numbers"}
                </Text>
              </View>
            }
          />
